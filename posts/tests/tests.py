...
class PostsLikeModelsTest(TestCase):
    ...
    def test_user_can_like_post_only_once(self):
        like1 = Like.objects.create(post=self.post1, user=self.user1)
        self.assertNotEqual(like1, None)

        with self.assertRaises(IntegrityError):
            like2 = Like.objects.create(post=self.post1, user=self.user1)
    ...
    def test_autonow_add_to_like(self):
        self.assertEqual(
            Like
            .objects
            .create(user=self.user1, post=self.post1)
